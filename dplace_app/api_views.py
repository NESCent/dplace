...
def result_set_from_query_dict(query_dict):
    ...
    #trees = models.LanguageTree.objects.filter(taxa__societies__id__in=[s.society.id for s in result_set.societies]).prefetch_related('taxa__languagetreelabelssequence_set__labels','taxa__languagetreelabelssequence_set__society').distinct()
    #labels = models.LanguageTreeLabels.objects.filter(societies__id__in=[s.society.id for s in result_set.societies]).select_related('languageTree')
    #for society in result_set.societies:
        #print labels.filter(societies__id__in=[society.society.id]).values('languageTree__id').distinct().count()
    #    result_set.set_tree_count(society.society, labels.filter(societies__id__in=[society.society.id]).values('languageTree').distinct().count())
    print "%s" % (time() - _s)
